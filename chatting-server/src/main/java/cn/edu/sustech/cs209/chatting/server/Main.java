...
//                    transferMessage.setSendToUrl(this.mainServer.clientPoor.get(message.getSendToUser()).socket.getLocalAddress().toString());
        ...
        } else if (message.getType() == 7) { //用户发给用户的文字消息，服务器负责转发
          System.out.println(Thread.currentThread().getName() + " | 服务端收到客户端发给" + message.getSendToUser() + "的确认已读信息：" + message.getData());
          for (Message mess : this.mainServer.HistoryMessage) {//
            if (mess.getBelongToUser().equals(message.getBelongToUser()) && mess.getBelongToChat().equals(message.getBelongToChat())) {
              if (mess.getTimestamp() < message.getTimestamp()) {
                mess.setHaveRead(true);
              }
            }
          }
        }
